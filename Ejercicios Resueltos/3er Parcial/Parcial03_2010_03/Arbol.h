...
class Arbol
{
	...
	public:
		...
		bool isCompleto( NodoA<T>*  ) const; // determina si el arbol es completo 
...
template<class T> bool Arbol<T>::isCompleto( NodoA<T>* recor  ) const
{
	bool completo = false;
	...
	if( recor->isHoja() && recor->getNivel() == this->altura )
		return true;
	else if( recor->isHoja() && recor->getNivel() != this->altura)
		return false;
	...
	// si el NodoA recor tiene hijo por la izquierda 
	if ( recor->getLink_hi() ) // paso recursivo para actualizar al hijo por la izquierda
		completo = isCompleto( recor->getLink_hi() );
	...
	// si el NodoA recor tiene hijo por la derecha
	if ( recor->getLink_hd() ) // paso recursivo para actualizar al hijo por la derecha
		completo = isCompleto( recor->getLink_hd() );
